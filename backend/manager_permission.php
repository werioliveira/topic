    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            <a href="/backend/register_module.php" class="nav-link">
              <i class="nav-icon far fa-circle text-info"></i>
              <p>Register Module</p>
            </a>
          </li>
          <li class="nav-item">
            <a href="/backend/manager_permission.php" class="nav-link">
              <i class="nav-icon far fa-circle text-info"></i>
              <p>Manager User Permission</p>
            </a>
          </li>
          <li class="nav-item">
            <a href="/backend/permission_module.php" class="nav-link">
              <i class="nav-icon far fa-circle text-info"></i>
      ...
      <div class="row">
        <div class="col-md-6">
          <div class="card card-primary">
            ...
            <form action="/backend/manager_post.php" method="post">
            <div class="card-body">
            <div class="form-group">
                <label for="inputUser">Usuario</label>
                <select name="inputUser"id="inputUser" class="form-control custom-select">
                  <option selected disabled>Select one</option>
                    <?php

                    include 'database.php';
                    $sql = "SELECT id, login FROM aula_php.usuario";  
              ...
              <div class="form-group">
              <label for="inputModule">Modulo</label>
              <select name="inputModule" id="inputModule" class="form-control custom-select">
                  <option selected disabled>Select one</option>
                    <?php
                    $sql = "SELECT id, titulo FROM aula_php.modulo";  
                    $result = $conn->query($sql);

                    if($result->num_rows > 0){
                        while($row = $result->fetch_assoc()){
                            echo "<option name ='module' value = '{$row['id']}'";
                            if ($selected_id == $row['id'])
                                echo "selected = 'selected'";
              ...
              <select name="inputPermission"id="inputPermission" class="form-control custom-select">
                  <option selected disabled>Select one</option>
                    <?php

                    $sql = "SELECT * FROM aula_php.permissao";  
                    $result = $conn->query($sql);

                    if($result->num_rows > 0){
                        while($row = $result->fetch_assoc()){
                            echo "<option name ='permission' value = '{$row['id']}'";
                            if ($selected_id == $row['id'])
                                echo "selected = 'selected'";
                            echo ">{$row['id']}</option>";
                        }
